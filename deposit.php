<?php
    ...
    $rqt2 = "SELECT a.account_number, a.account_id, a.balance, c.full_name, c.email, c.phone FROM customers c JOIN accounts a WHERE c.customer_id = a.customer_id";
    $result2 = mysqli_query($connexion, $rqt2);
    $customers = mysqli_fetch_all($result2, MYSQLI_ASSOC);
    
    if(isset($_GET['id'])){
        $id_to_deposit = $_GET['id'];
        $rqt3 = "SELECT a.account_number, a.account_id, a.balance, c.full_name, c.email, c.phone FROM customers c JOIN accounts a WHERE c.customer_id = a.customer_id AND account_id= '$id_to_deposit'";
        $result3 = mysqli_query($connexion, $rqt3);
        $customers_infos = mysqli_fetch_assoc($result3);
    }
    if(isset($_POST['add_action'])){
        $amount_to_add = $_POST['t_balance'];
        $old_balance = $customers['balance'];
        $rqt4 = "UPDATE acoounts balance = '$old_balance' + '$amount_to_add'";
        if( mysqli_query($connexion, $rqt4)){
            echo 'deposit added successufully';
            header("location: tran_direct.php");
        }
        else{
            die('Deposit Error : '. mysqli_error($connexion));
...
<body>
    <div class="module_customers" id="module_customers" style="display:block;">
        ...
        <h2 id="module_msg">NEW DEPOSIT</h2>
        <form action="transactions.php" method="post">
            <div class="module_customers_inputs">
            <div>
                <input type="text" placeholder="Account Number" name="t_a_n1" value="<?php echo $customers_infos['account_number']; ?>" readonly>
            </div>
            <div>
                <input type="text" placeholder="Owner Name" name="t_a_n2" value="<?php echo $customers_infos['full_name']; ?>" readonly>
            </div>
            <div>
                <input type="tel" placeholder="Amount" name="t_balance">
            </div>
            <div>
                <input type="text" placeholder="Description (optional)" name="t_description">
            </div>
        </div>
        <div class="module_customers_btns">
            <button id="cancel_btn">Cancel</button>
            <button class="add_btn" id="add_btn" name="add_action">Add</button>
        </div>
        </form>
    </div>
            ...
            <table>
                ...
                <?php if(!empty($customers)): ?>
                    ...
                    <tr class="container_transaction">
                        <td><?php echo $customers['account_number'] ?></td>
                        <td><?php echo $customers['full_name'] ?></td>
                        <td><?php echo $customers['email'] ?></td>
                        <td><?php echo $customers['phone'] ?></td>
                        <td><?php echo $customers['balance'] ?> MAD</td>
                        <form action="deposit.php" method="get">
                            <td><a name="deposit" href="tran_direct.php?id=<?php echo htmlspecialchars($customers['account_id']); ?>">Deposit</a></td>
                            <td><a name="withdraw" href="tran_direct.php?id=<?php echo htmlspecialchars($customers['account_id']); ?>">Withdraw</a></td>
                        </form>
                    </tr>
                <?php endforeach; ?>
                <?php endif ;?>
